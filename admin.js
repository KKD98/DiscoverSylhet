...
button.addEventListener('click', (event) => {
    ...
    let travelCost = document.getElementById('travelCost').value
    let placeNumber = document.getElementById('placeingNumber').value
    let _style = document.getElementById('style').value
...
const uploadImage = async (file) => {
    ...
    const response = await fetch('https://discoversylhet.herokuapp.com/places/imageUpload', settings);
...
$('#upload_file').change(function(e) {
    ...
    $.ajax({
        url: 'https://discoversylhet.herokuapp.com/places/imageUpload',
        cache: false,
        contentType: false,
        processData: false,
        data: form_data,
...
const uploadData = async (data) => {
    ...
    const response = await fetch(' https://discoversylhet.herokuapp.com/places/createplace', settings);
    if (!response.ok) throw Error(response.message);
    try {
        const data = await response.json();
        alert("Data Uploaded");
